    ...
    public class Patcher
    {
        ...
        public static IEnumerable<CodeInstruction> FixMultiplayerFootstepDisplay(IEnumerable<CodeInstruction> instructions)
        {
            try
            {
                ...
                object branchDestination = instructionList[2].operand; // l[2] points to the first Brfalse_S
        ...
        public static IEnumerable<CodeInstruction> FixHeadAndHatPosition(IEnumerable<CodeInstruction> instructions)
        {
            try
            {
                ...
                {
                    if (!foundHead && instructionsList[i].opcode == OpCodes.Ldc_R4 && (float)instructionsList[i].operand >= 47.9f && (float)instructionsList[i].operand <= 48.1f)
                    {
                    ...
                    if (!foundHat && instructionsList[i].opcode == OpCodes.Call && instructionsList[i].operand.ToString().ToLower().Contains("get_zero"))
                    {
                        if (instructionsList[i + 1].opcode == OpCodes.Stloc_1 && instructionsList[i + 2].opcode == OpCodes.Ldarg_0)
                        {
        ...
        public static void DrawEmoteAndSaddleBags(ref Horse __instance, ref SpriteBatch b, ref bool __state)
        {
            ...
            if (horse.IsEmoting)
            {
                ...
                b.Draw(Game1.emoteSpriteSheet, emotePosition, new Microsoft.Xna.Framework.Rectangle?(new Rectangle((horse.CurrentEmoteIndex * 16) % Game1.emoteSpriteSheet.Width, horse.CurrentEmoteIndex * 16 / Game1.emoteSpriteSheet.Width * 16, 16, 16)), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, horse.StandingPixel.Y / 10000f);
            }
        }
